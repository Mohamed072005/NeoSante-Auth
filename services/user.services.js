...
exports.handelOTPCode = async (token, code, rememberMe, userAagent) => {
    try {
        ...
        if (rememberMe) {
            ...
            if (!alreadyHaveThisAgent) await createUserAgent(userAagent, null, user.id);
        }
        return {
...
exports.resetOTPService = async (user_id, userAgent) => {
    try {
        ...
        const JWTtoken = generateJWT({ code: OTPCode, user_id: user.id, user_email: user.email }, '120s');
